    ...
    while True:
        ...
        if best_angle is None:
            ...
            while best_angle is None:
                ...
                turn(
                    rel_to_abs(world_state.heading, wedgeSize * wedgeDist),
                    direction,
                    world_state,
                    ros_util,
                )
        ...
        while best_index <= min_index or best_index >= max_index:
            ...
            turn(
                rel_to_abs(world_state.heading, wedgeSize),
                direction,
                world_state,
                ros_util,
            )
...
def send_feedback(world_state, waypoint_server):
    ...
    current_pose = Pose()
    current_pose.position.x = float(world_state.positionX)
    current_pose.position.y = float(world_state.positionY)
    current_pose.position.z = float(world_state.positionZ)

    feedback = Waypoint.Feedback()
    feedback.pose = current_pose
    feedback.battery = world_state.battery
...
def build_result(world_state, preempted):
    ...
    result.pose.position.y = float(world_state.positionY)
    result.pose.position.z = float(world_state.positionZ)
